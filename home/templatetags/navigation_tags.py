...
def get_lang_root(context, page=None):
    # This returns the language code for this page.
    language_homepage = page.get_ancestors(inclusive=True).get(depth=3)
    return language_homepage
...
@register.inclusion_tag('includes/sub_pages.html', takes_context=True)
def sub_pages(context, parent, calling_page=None):
    sub_pages = calling_page.get_children().live()
    return {
        'sub_pages': sub_pages,
        'request': context['request']
    }
...
@register.inclusion_tag('includes/top_menu_children.html', takes_context=True)
